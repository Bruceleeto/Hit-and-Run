...
// Description: Dreamcast controller implementation using KOS maple.
//              Exposes same radController API so the game links.
//=============================================================================
...
{
    { g_Dcipt[0], "DPadUp",        0 },
    { g_Dcipt[0], "DPadDown",      1 },
    { g_Dcipt[0], "DPadLeft",      2 },
    { g_Dcipt[0], "DPadRight",     3 },
    { g_Dcipt[0], "Start",         4 },
    { g_Dcipt[0], "Back",          5 },
    { g_Dcipt[0], "A",             6 },
    { g_Dcipt[0], "B",             7 },
    ...
    { g_Dcipt[0], "Black",        10 },
    { g_Dcipt[0], "White",        11 },
    { g_Dcipt[1], "LeftTrigger",  12 },
    { g_Dcipt[1], "RightTrigger", 13 },
    { g_Dcipt[2], "LeftStickX",   14 },
    { g_Dcipt[3], "LeftStickY",   15 },
    { g_Dcipt[2], "RightStickX",  16 },
    { g_Dcipt[3], "RightStickY",  17 }
};
...
public:
    ...
    radControllerOutputPointDC( const char* pName )
        : radRefCount(0), m_pName(pName), m_Gain(0.0f)
    {
...
public:
    ...
    float CalculateNewValue( void )
    {
        ...
        switch( m_Identifier )
        {
            case  0: return (state->buttons & CONT_DPAD_UP)    ? 1.0f : 0.0f;
            case  1: return (state->buttons & CONT_DPAD_DOWN)  ? 1.0f : 0.0f;
            case  2: return (state->buttons & CONT_DPAD_LEFT)  ? 1.0f : 0.0f;
            case  3: return (state->buttons & CONT_DPAD_RIGHT) ? 1.0f : 0.0f;
            case  4: return (state->buttons & CONT_START)      ? 1.0f : 0.0f;
            case  5: return 0.0f; // Back - DC has no back button
            case  6: return (state->buttons & CONT_A) ? 1.0f : 0.0f;
            case  7: return (state->buttons & CONT_B) ? 1.0f : 0.0f;
            case  8: return (state->buttons & CONT_X) ? 1.0f : 0.0f;
            case  9: return (state->buttons & CONT_Y) ? 1.0f : 0.0f;
            case 10: return 0.0f; // Black (L shoulder) - not on DC
            case 11: return 0.0f; // White (R shoulder) - not on DC
            case 12: return (float)state->ltrig / 255.0f; // Left trigger
            case 13: return (float)state->rtrig / 255.0f; // Right trigger
            case 14: return (float)state->joyx  / 128.0f; // Left stick X
            case 15: return (float)state->joyy  / 128.0f; // Left stick Y
            case 16: return (float)state->joy2x / 128.0f; // Right stick X
            case 17: return (float)state->joy2y / 128.0f; // Right stick Y
            default: return 0.0f;
        }
    }
    ...
    virtual void RegisterControllerInputPointCallback(
        IRadControllerInputPointCallback* pCallback, unsigned int userData = 0 )
    {
        ...
        IRadControllerInputPointCallback* pCallback )
    {
        ...
        if ( (pIWcr = reinterpret_cast<IRadWeakCallbackWrapper*>(
                m_xIOl_Callbacks->GetNext() )) )
        {
    ...
    radControllerInputPointDC( const char* pType, const char* pName, int id )
        : radRefCount(0), m_Value(0.0f), m_MinRange(0.0f), m_MaxRange(1.0f),
          m_Tolerance(0.0f), m_TimeInState(0), m_TimeOfStateChange(0),
          m_pType(pType), m_pName(pName), m_Identifier(id)
    {
    ...
    ~radControllerInputPointDC( void )
    {
        rAssertMsg( m_xIOl_Callbacks->GetSize() == 0,
            "Someone forgot to UnRegister an input point callback" );
    }
...
public:
    ...
    {
        (void)virtualTime;
        // Update all input point values from maple
        m_xIOl_InputPoints->Reset();
        radControllerInputPointDC* p;
        while ( (p = reinterpret_cast<radControllerInputPointDC*>(
                m_xIOl_InputPoints->GetNext() )) )
        {
            float newVal = p->CalculateNewValue();
            float diff = newVal - p->m_Value;
            if( diff < 0.0f ) diff = -diff;
            if( diff > p->m_Tolerance )
            {
                p->m_Value = newVal;
                p->m_TimeInState = 0;
                // Fire callbacks
                p->m_xIOl_Callbacks->Reset();
                IRadWeakCallbackWrapper* pCb;
                while( (pCb = reinterpret_cast<IRadWeakCallbackWrapper*>(
                        p->m_xIOl_Callbacks->GetNext() )) )
                {
                    IRadControllerInputPointCallback* cb =
                        (IRadControllerInputPointCallback*)pCb->GetWeakInterface();
                    if( cb ) cb->OnControllerInputPointChange(
                        (unsigned int)(uintptr_t)pCb->GetUserData(), newVal );
                }
            }
        }
    }
    ...
    virtual void iVirtualTimeReMapped( unsigned int virtualTime )
    {
        ...
        while ( (p = reinterpret_cast<IRadControllerInputPointDC*>(
                m_xIOl_InputPoints->GetNext() )) )
    ...
    {
        (void)milliseconds; (void)pollingRate;
    }
    ...
    virtual unsigned int GetNumberOfInputPointsOfType( const char* pType )
    {
        ...
        while ( (p = reinterpret_cast<IRadControllerInputPoint*>(
                m_xIOl_InputPoints->GetNext() )) )
    ...
    virtual IRadControllerInputPoint* GetInputPointByTypeAndIndex(
        const char* pType, unsigned int indx )
    {
        unsigned int i = 0;
        m_xIOl_InputPoints->Reset();
        IRadControllerInputPoint* p;
    ...
    {
        (void)pType;
        return reinterpret_cast<IRadControllerOutputPoint*>(
            m_xIOl_OutputPoints->GetAt( indx ) );
    }
    ...
    virtual IRadControllerOutputPoint* GetOutputPointByName( const char* pName )
    {
        ...
        {
            IRadControllerOutputPoint* p =
                reinterpret_cast<IRadControllerOutputPoint*>(
                    m_xIOl_OutputPoints->GetAt( i ) );
    ...
    {
        return reinterpret_cast<IRadControllerInputPoint*>(
            m_xIOl_InputPoints->GetAt( index ) );
    }
    ...
    {
        return reinterpret_cast<IRadControllerOutputPoint*>(
            m_xIOl_OutputPoints->GetAt( index ) );
    }
    ...
    radControllerDC( unsigned int thisAllocator, unsigned int virtualTime,
                     unsigned int bufferTime, unsigned int pollingRate )
        : radRefCount(0)
    {
        ...
        {
            ref< radControllerInputPointDC > pInputPoint =
                new( g_ControllerSystemAllocator ) radControllerInputPointDC(
                    g_DCPoints[i].m_pType, g_DCPoints[i].m_pName,
                    (int)g_DCPoints[i].m_Id );
            m_xIOl_InputPoints->AddObject( pInputPoint );
            pInputPoint->iInitialize();
        }

        radControllerOutputPointDC* pLeft =
            new( g_ControllerSystemAllocator ) radControllerOutputPointDC( "LeftMotor" );
        radControllerOutputPointDC* pRight =
            new( g_ControllerSystemAllocator ) radControllerOutputPointDC( "RightMotor" );
...
public:
    ...
    virtual void OnTimerDone( unsigned int elapsedtime, void* pUserData )
    {
        ...
        IRadControllerDC* p;
        while ( (p = reinterpret_cast<IRadControllerDC*>(
    ...
    {
        return reinterpret_cast<IRadController*>(
            m_xIOl_Controllers->GetAt( myindex ) );
    }
    ...
    virtual IRadController* GetControllerAtLocation( const char* pLocation )
    {
        ...
        m_xIOl_Controllers->Reset();
        IRadController* pIC2;
        while ( (pIC2 = reinterpret_cast<IRadController*>(
                m_xIOl_Controllers->GetNext() )) )
        {
        ...
        IRadControllerConnectionChangeCallback* pCallback )
    {
        ...
        while ( (pIWir = reinterpret_cast<IRadWeakInterfaceWrapper*>(
                m_xIOl_Callbacks->GetNext() )) )
        {
        ...
        rAssertMsg( false,
            "Controller connection change callback not registered." );
    }

    void Service( void )
    {
        if ( m_xITimerList != NULL )
    ...
    radControllerSystemDC(
        ...
        : m_UsingVirtualTime(false), m_VirtualTimeAdjust(0),
          m_EventBufferTime(0), m_DefaultConnectionChangeCallback(NULL)
    {
        radMemoryMonitorIdentifyAllocation( this, g_nameFTech,
            "radControllerSystemDC" );
        ...
        // Create one controller for port 0
        ref< IRadController > xIController2 =
            new ( g_ControllerSystemAllocator ) radControllerDC(
                g_ControllerSystemAllocator,
                radTimeGetMilliseconds() + m_VirtualTimeAdjust,
                m_EventBufferTime, 10 );
        ...
        {
            IRadControllerConnectionChangeCallback* pCallback =
                (IRadControllerConnectionChangeCallback*)pIWir->GetWeakInterface();
